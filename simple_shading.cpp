...
Vec3Df simple_diffuse(Vec3Df hitpoint, Vec3Df lightPos, Vec3Df normal, Material material){
...
Vec3Df simple_specular(Vec3Df hitpoint, Vec3Df lightPos, Vec3Df cameraPos, Vec3Df normal, Material material){
	Vec3Df light_vector = lightPos-hitpoint;
...
Vec3Df shoot_ray(Ray ray, Scene scene, int bounce_limit){
	...
	if(ray.type == PRIMARY_RAY){
		...
		if(h.isHit == 1){
			...
			Colour += simple_diffuse(ray.previous_hit.hitPoint, ray.dest, ray.previous_hit.normal, ray.previous_hit.material)/3;
			Colour += simple_specular(ray.previous_hit.hitPoint, ray.dest, MyCameraPosition, ray.previous_hit.normal, ray.previous_hit.material)/3;
...
Vec3Df simple_shading_routine(Scene scene, Vec3Df origin, Vec3Df dest){
	Vec3Df colour = black;
